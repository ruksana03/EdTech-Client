    ...
    return (
        <div className='section-container w-full min-h-[calc(100vh-40px)]' >
            ...
            <form className='flex items-center flex-col md:flex-row lg:flex-row justify-start gap-8 w-full my-5 px-5'>
                <select onChange={() => setSearch2(event.target.value)} className=" w-44 border border-gray-300 dark:text-gray-300 dark:bg-zinc-700 dark:border text-black focus:outline-none focus:bg-white focus:border-first leading-tight input" name='select'>
                ...
                <div className='relative w-full'><input type="text" onChange={() => setSearch(event.target.value)} className='pl-10 text-[17px]dark:bg-zinc-600 appearance-none input border-2 text-[17px] border-gray-200 dark:bg-zinc-700 dark:border rounded w-full md:w-2/3 lg:w-1/3 py-4 px-4 leading-tight focus:outline-none focus:bg-white focus:border-first' name='search' placeholder=' type here...' /> <MdOutlineSearch className='absolute top-2 left-1 text-3xl' /> </div>
            </form>
