...
const NoticeHome = () => {
  ...
  useEffect(() => {
    const searchItem = notices.filter((item) =>
      ...
      <div className="section-container">
        <div className="flex items-center flex-col-reverse md:flex-row gap-5 lg:flex-row justify-between w-full my-5">
          <div className="flex items-center gap-3 dark:text-gray-400">
            <h1>Show</h1>
            <select
              onChange={() => setSettingPage(event.target.value)}
              className="w-32 border border-gray-300 dark:bg-zinc-800 dark:text-gray-400 text-black focus:outline-none focus:bg-white focus:border-first leading-tight input"
              name="select"
            >
          ...
          <div className="relative w-[350px] flex items-center gap-1 dark:text-gray-400">
            ...
            <input
              ...
              className="text-[17px]dark:bg-zinc-600 dark:text-gray-400 dark:bg-zinc-800 appearance-none input border-2 text-[17px] border-gray-200 rounded w-[80%] md:w-full lg:w-full py-4 px-4 leading-tight focus:outline-none focus:bg-white focus:border-first"
              name="search"
            ...
            <tbody>
              {currentItems?.length > 0 &&
                currentItems?.map((notice) => (
                  <tr
                    key={notice._id}
                    className="border hover:bg-base-300 dark:hover:bg-third cursor-pointer"
                  >
                    <td className="border text-base lg:text-[18px] font-medium hover:dark:text-black text-gray-800 dark:text-gray-400">
                      <Link to={`/notice-details/${notice?._id}`}>
                        {notice?.title}
                      </Link>
                    </td>
                    <td className="border text-gray-800 dark:text-gray-400 font-medium ">
                      All Teacher
                    </td>
                    <td className="border text-gray-800 dark:text-gray-400 font-medium ">
                      10-03-2023
